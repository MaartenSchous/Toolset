...
namespace _07_Toolset
{
    public class Toolset
    {
        ...
        //
        //Looking for trouble
        //RESTful architecture favors JSON. I ran into a JSON parsing problem once and wanted to be able to save the payloads to disk for inspection.

        /* Example JSON
        var parameters = new StableDiffusionParameters
        {
            prompt = "This and that,",
            steps = 30,
            width = 512,
        };
        ...
        */
        ...
        //I used this to have a look at an eronous JSON payloads.
        public void SaveJsonToTextFile(string jsonContent, string saveDirectory)
        {
        // The name of the file
        const string fileName = "payload.json"; // Using .json extension is standard, even if saved as text

        try
        {
        // Combine the directory path and the filename to get the full path.
        // Path.Combine is safer than string concatenation for handling slashes.
        string fullPath = Path.Combine(saveDirectory, fileName);

        // Write the entire JSON string to the file.
        // This handles creating the file if it doesn't exist or overwriting it if it does.
        File.WriteAllText(fullPath, jsonContent);

        // Optionally, show a confirmation or log the success
        // MessageBox.Show($"JSON successfully saved to:\n{fullPath}", "Save Successful");
    }
    catch (DirectoryNotFoundException)
    {
        // Handle the case where the saveDirectory does not exist.
        // We can create the directory and then try saving again.
        Directory.CreateDirectory(saveDirectory);
        SaveJsonToTextFile(jsonContent, saveDirectory); // Recursive call to save now
    }
    catch (Exception ex)
    {
        // Handle other file access or IO errors (permissions, disk space, etc.)
        // MessageBox.Show($"Error saving JSON file: {ex.Message}", "File Save Error");
    }
}
